import {
  ...
  export class SelectedStudentsController {
    ...
    @response(201, {
      ...
      content: {'application/json': {schema: getModelSchemaRef(SelectedStudents)}},
    })
    async create(
      @requestBody({
        content: {
    ...
    async replaceByEnrollmentNumber(
      ...
      if (!existing) {
        throw new Error(`SelectedStudent with enrollment_number ${enrollment_number} not found`);
      }
      await this.selectedStudentsRepository.replaceById(existing.id!, selectedStudents);
    }
